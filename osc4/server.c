...
int main(int argc, char *argv[])
{
  ...
  pthread_create(&tid[0], &attr[0], server, &st1); 
  pthread_attr_init(&attr[1]);
  pthread_create(&tid[1], &attr[1], server, &st2); 
  pthread_attr_init(&attr[2]);
  pthread_create(&tid[2], &attr[2], server, &st3);
...
void *server(void *temp){//生产者
    ...
    if(st.number ==1) pipef= open(PIPE1, O_RDONLY|O_NONBLOCK);
    else if(st.number ==2) pipef= open(PIPE2, O_RDONLY|O_NONBLOCK);
    else if(st.number ==3) pipef= open(PIPE3, O_RDONLY|O_NONBLOCK);
    if(pipef < 0)
    {
