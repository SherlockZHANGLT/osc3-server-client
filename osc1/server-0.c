...
const char *pipe1 = "./pipe4";
const char *pipe2 = "./pipe5";
const char *pipe3 = "./pipe6";
const char *pipe_sc = "./pipe_sc";
...
int main(int argc, char *argv[])
{
    ...
    //pipe
    pipef[0] = open(pipe1, O_WRONLY);
    if(pipef[0] < 0)
    {
	printf("open pipef4 error is %s\n", strerror(errno));
	return -1;
    }
    pipef[1] = open(pipe2, O_WRONLY);
    if(pipef[1] < 0)
    {
	printf("open pipef5 error is %s\n", strerror(errno));
	return -1;
    }
    pipef[2] = open(pipe3, O_WRONLY);
    if(pipef[2] < 0)
    {
...
void *server(void *temp){//生产者
    ...
    int pipef;
    if(p_c == 0){pipef= open(pipe1, O_RDONLY);p_c++;}
    else if(p_c == 1){pipef= open(pipe2, O_RDONLY);p_c++;}
    else{pipef= open(pipe3, O_RDONLY);}
